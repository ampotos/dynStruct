...
def get_args():
    ...
    parser.add_argument('-k', dest='run_cleaning', action='store_false',
                        help='don\'t remove block which look like array after\
                        structure recovery')
...
def load_json(json_data, l_block, l_access_w, l_access_r):
    ...
    try:
        prbar = pyprind.ProgBar(
            len(json_data["blocks"]), track_time=False, title="Loading Json data")
        for block in filter(None, json_data["blocks"]):
            l_block.append(_dynStruct.Block(
                block, l_access_w, l_access_r, id_block))
            id_block += 1
            prbar.update()
...
def main():
 ...
 data extract by the gatherer or serialized data from a previous run")
    ...
    if args.dynamo_file:
        ...
        f.close()
        load_json(json_data, _dynStruct.l_block,
                  _dynStruct.l_access_w, _dynStruct.l_access_r)
        if args.run_recovering:
            _dynStruct.Struct.recover_all_struct(
                _dynStruct.l_block, _dynStruct.l_struct)
            _dynStruct.Struct.clean_all_struct(
                _dynStruct.l_struct, args.run_cleaning)
    ...
    if args.out_pickle:
        _dynStruct.save_pickle(args.out_pickle, _dynStruct.l_struct,
    ...
    if args.web_view:
        _dynStruct.start_webui(args.bind_addr.split(":")[0],
                               args.bind_addr.split(":")[1] if ":" in
                               args.bind_addr else 24242)
